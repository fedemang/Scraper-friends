# -*- coding: utf-8 -*-
"""
Created on Tue Oct 22 15:05:57 2019

@author: Federico
"""

# -*- coding: utf-8 -*-
"""
Created on Tue Oct 15 12:26:00 2019

@author: Federico
"""

#Scrapes a websites for any tables, sorting the relevant data (ex: names, tlf numbers, titles) 
#and emails, creating a compact dataset

from bs4 import BeautifulSoup
import requests
#import pandas as pd
import smtplib, ssl
from email.mime.text import MIMEText
from email.mime.multipart import MIMEMultipart

source = requests.get('https://eps.leeds.ac.uk/chemistry/stafflist/').text
soup = BeautifulSoup(source, 'lxml')
#print(soup)

replaceString = "" # replace each \n tag with ""
cleansoup = BeautifulSoup(str(soup).replace("\n", replaceString),'lxml')
cleansoup = BeautifulSoup(str(soup).replace("\xa0", replaceString),'lxml')

#imported_table=cleansoup.find('table',{'class':'wikitable sortable'}) #only works for wikipedia!
imported_table=cleansoup.find_all('table')
email = cleansoup.select('a[href^=mailto]')
#print(imported_table)

#headers = imported_table.find_all('th')
#datas = imported_table.find_all('tr')



Mail=[]

for i in email:
    href=i['href']
    try:
        str1, str2 = href.split(':')
    except ValueError:
        break
        
    Mail.append(str2)


Data=[]

for tbody in imported_table:
    tbody=tbody.find_all('tr')
    for tr in tbody:
        td=tr.find_all('td')
        row=[i.text for i in td]
        Data.append(row)
    #print(row)
    #print(row)

Data = [x for x in Data if x] #gets rid of everything that is empty in the Data list
 
Mail = [x for x in Mail if x]    


# creates separate lists from a list of lists

dummy=len(Data)

Information1=[]
for i in range(0,dummy):
    Information1.append(Data[i][0])
  
Information2=[]
for i in range(0,dummy):
    Information2.append(Data[i][1])  
    
Information3=[]
for i in range(0,dummy):
    Information3.append(Data[i][2]) 
 
Information4=[]
for i in range(0,dummy):
    Information4.append(Data[i][3]) 
    

#Information1 = [x for x in Information1 if x]  
#Information4 = [x for x in Information4 if x]  

for i in range(6,dummy): 
    Information1[i] = Information2[i] + " " + Information3[i]

#Mail = Information4  
 
"""     
#create a single dataframe by concatanating previously created dataframes
df1=pd.DataFrame() 
df1["Name"]= Information1
df2=pd.DataFrame() 
df2["Number"] = Information2
df3=pd.DataFrame() 
df3["Department"] = Information3
df4=pd.DataFrame() 
df4["Extra"] = Information4
df5=pd.DataFrame()
df5["E-Mail"] = Mail
result = pd.concat([df1, df2, df3, df4, df5], axis=1, sort=False)
"""

sender_email = "federico.masini11@gmail.com"#
#sender_email = "uk@cphnano.com"#
#receiver_email = "fedemang@gmail.com"
password = input("Type your password and press enter:")
#password = "password"


for i in range (0, len(Data)):
    receiver_email= Mail[i]
    message = MIMEMultipart("alternative")
    message["Subject"] = "Presentation of NanoCuvette™"
    message["From"] = sender_email
    message["To"] = receiver_email
    
    
    # Create the plain-text and HTML version of your message
    #text = """\Hey"""
    
    
    html = """\
    <html>
      <body>
        <p>Dear {code}<br>
           I send you this mail on behalf of the company Copenhagen Nanosystems ApS.<br>
           Copenhagen Nanosystems ApS is a young company which produces NanoCuvette™ One,<br> 
           an innovative product that can turn your Spectrophotometer in a much more powerful tool,<br> 
           without instrument modification and without buying new costly instruments.<br>
           More on our business on our website https://cphnano.com/<br>  
           The NanoCuvette™ One is PMMA plastic cuvette that embeds an anti-stiction coated (ASC) optical filter <br>  
           based on nanotechnology. The cuvette can measure refractive index, using visual light (550 nm to 800 nm),<br>   
           and absorbance in the UV-Vis range (300 nm to 800 nm).<br> 
           Due to its unique characteristics, NanoCuvette™ is the ideal product for the following tasks:<br>
           - Label-free enzyme kinetics<br>
           - Label-free protein concentration measurements<br>
           - Label-free DNA concentration measurements<br>
           - Quality control<br>
           - Droplet measurements (i.e. accurate measurements on 0.5 μL droplets)<br>.
           Furthermore, the company provides in-cloud data processing and storing: the software is user-friendly,<br>
           fast and hosted by Amazon Web Services (AWS), which ensures that the customers data is handled reliably,<br>
           privately and securely.<br>
           If this could be interesting for you, I would love to meet you <br> 
           (or whoever in your company/department could be interested in the product)<br> 
           at your convenience, to showcase the product capability and discuss possible purchase.<br>
           Best regards,<br>
           Federico Masini<br>
        </p>
      </body>
    </html>
    """.format(code=Information1[i])
    
    # Turn these into plain/html MIMEText objects
   # part1 = MIMEText(text, "plain")
    part2 = MIMEText(html, "html")
    
    # Add HTML/plain-text parts to MIMEMultipart message
    # The email client will try to render the last part first
   # message.attach(part1)
    message.attach(part2)
 
    # Creates secure connection with server and sends email
    try:
        context = ssl.create_default_context()
        with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as server:
            server.login(sender_email, password)
            server.sendmail(
                    sender_email, receiver_email, message.as_string()
                    )
            print ('email sent')
    except:
        print ('error sending email')
        
print('task completed')

